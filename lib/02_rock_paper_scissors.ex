defmodule AdventOfCode.RockPaperScissors do
  ...
  """
  @spec calculate_score(String.t(), []) :: pos_integer()
  def calculate_score(data, opts \\ []) do
    ...
    |> Enum.reduce(0, fn round, acc ->
      acc + round_score(round)
    end)
  end

  defp round_score(["A", "X"]), do: 4
  defp round_score(["A", "Y"]), do: 8
  defp round_score(["A", "Z"]), do: 3

  defp round_score(["B", "X"]), do: 1
  defp round_score(["B", "Y"]), do: 5
  defp round_score(["B", "Z"]), do: 9
